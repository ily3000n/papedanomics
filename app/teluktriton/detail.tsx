...
const Detail: React.FC = () => {
  ...
  return (
    <div className='bg-[#13182B] h-full pb-[6rem]'>
      <h1 className='text-center text-4xl font-extrabold pt-6 text-white'>Teluk Triton</h1>
      <div className='flex flex-col md:flex-row gap-16 p-4 mx-4'>
        <div className="w-full md:w-5/12 md:mx-16">
          ...
          >
            <CarouselContent>
              {dataWisata[3].slide.map((img, index) => (
        ...
        <div className="w-full md:w-1/2 text-white mx-4 md:mx-0">
          <h2 className="text-2xl font-bold justify-center">{dataWisata[3].title}</h2>
          <p className='mt-9 text-justify'>{dataWisata[3].des}</p>
        </div>
      </div>
    </div>
  )
